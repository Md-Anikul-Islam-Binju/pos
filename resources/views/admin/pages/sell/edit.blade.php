...
<div class="row">
    <div class="col-12">
        <div class="page-title-box d-flex align-items-center justify-content-between">
            <h4 class="mb-0">Edit Sell #{{ $sell->unique_sale_id }}</h4>
...
<form action="{{ route('sells.update', $sell->id) }}" method="POST" id="sellForm">
    ...
    <div class="mb-3 d-flex align-items-center gap-2">
        <label for="currency" class="me-2">Select Currency</label>
        <select id="currency" class="form-control" required>
            @foreach($currencies as $currency)
                <option value="{{ $currency->id }}" {{ $sell->currency_id == $currency->id ? 'selected' : '' }}>
        ...
        <div class="col-md-4 mb-3">
            ...
            <select name="customer" id="customer" class="form-control" required>
                ...
                @foreach($customers as $customer)
                    <option value="{{ $customer->id }}" {{ $sell->customer_id == $customer->id ? 'selected' : '' }}>{{ $customer->name }}</option>
                @endforeach
            </select>
        </div>
        <div class="col-md-4 mb-3">
            ...
            <select name="salesman" id="salesman" class="form-control" required>
                ...
                @foreach($salesman as $user)
                    <option value="{{ $user->id }}" {{ $sell->salesman_id == $user->id ? 'selected' : '' }}>{{ $user->name }}</option>
                @endforeach
            </select>
        </div>
        <div class="col-md-4 mb-3">
            ...
            <select name="account_id" id="account" class="form-control" required>
                ...
                @foreach($accounts as $account)
                    <option value="{{ $account->id }}" {{ $sell->account_id == $account->id ? 'selected' : '' }}>{{ $account->name }}</option>
                @endforeach
            </select>
        </div>
    </div>
    ...
    <table class="table table-bordered" id="productsTable">
        ...
        <tbody>
            @foreach($existingProducts as $item)
                <tr>
                    <td>
                        <select name="stock_id[]" class="form-control stock_select">
                            @foreach($stockProducts as $p)
                                <option value="{{ $p->id }}" {{ $item->stock_id == $p->id ? 'selected' : '' }} data-price="{{ $p->price }}" data-qty="{{ $p->quantity }}">
                                    {{ $p->product->name }}
                                </option>
                            @endforeach
                        </select>
                    </td>
                    <td class="available_qty">{{ $item->stock->quantity }}</td>
                    <td><input type="number" name="product_price[]" class="form-control product_price" value="{{ $item->price }}" min="0"></td>
                    <td><input type="number" name="product_quantity[]" class="form-control product_quantity" value="{{ $item->quantity }}" min="1"></td>
                    <td>
                    ...
                    <td><input type="number" name="discount_amount[]" class="form-control discount_amount" value="{{ $item->discount_amount }}" min="0"></td>
    ...
    <div class="row mb-3">
        <div class="col-md-6">
            ...
            <select name="paidAmountOption" id="paidAmountOption" class="form-control">
                <option value="paid_in_full" {{ $sell->paidAmountOption == 'paid_in_full' ? 'selected' : '' }}>Paid in Full</option>
                <option value="custom_amount" {{ $sell->paidAmountOption == 'custom_amount' ? 'selected' : '' }}>Custom Amount</option>
            </select>
        </div>
        <div class="col-md-6">
            <label>Paid Amount</label>
            <input type="number" name="amount" id="paid_amount_input" class="form-control" value="{{ $sell->amount }}" {{ $sell->paidAmountOption == 'custom_amount' ? '' : 'disabled' }}>
        </div>
    </div>
...
// Handle changes
$(document).on('change', '.stock_select, .product_price, .product_quantity, .discount_type, .discount_amount', updateTotals);
...
$('#product_category').change(function() {
    ...
    $.get("{{ route('sells.products.by.category') }}", { category_id: categoryId }, function(res) {
...
// Set initial totals on page load
updateTotals();
</script>
@endsection
